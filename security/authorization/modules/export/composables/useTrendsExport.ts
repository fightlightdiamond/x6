/**
 * Composable for exporting Trends data in various formats.
 *
 * Checks `export` permission on `trends_export` resource before performing
 * any export operation. Manages loading, success, and error states reactively.
 *
 * @module modules/export/composables/useTrendsExport
 */

import { ref } from "vue";
import { useAuthorizationStore } from "../../../stores/authorizationStore";
import { trendsApiService } from "../../trends/api";

/** Supported export formats. */
export type ExportFormat = "csv" | "json" | "excel";

/**
 * Composable that provides trend export functionality with permission checking,
 * loading state, success message, and error message.
 *
 * @example
 * ```ts
 * const { loading, success, error, exportTrend } = useTrendsExport();
 * await exportTrend('trend-123', 'csv');
 * ```
 */
export function useTrendsExport(): {
  ...
  exportTrend: (trendId: string, format: ExportFormat) => Promise<void>;
} {
  ...
  /**
   * Exports a single trend by id in the specified format.
   *
   * Checks `export` permission on `trends_export` before fetching data.
   * Sets `loading`, `success`, and `error` states accordingly.
   *
   * @param trendId - The id of the trend to export.
  ...
  async function exportTrend(
    ...
    if (!authStore.can("export", "trends_export")) {
      error.value = "Permission denied: cannot export trends";
      return;
    }
    ...
    try {
      const trend = await trendsApiService.getById(trendId);

      if (!trend) {
        error.value = "Trend not found";
        return;
      }

      // Convert to the requested format and trigger browser download
      const content = convertToFormat(trend, format);
      downloadFile(content, `trend-${trendId}.${getExtension(format)}`, format);

      success.value = `Trend exported successfully as ${format.toUpperCase()}`;
    } catch (err) {
  ...
  return { loading, success, error, exportTrend };
}
